...
The page registers 12 tools with the browser via `document.modelContext.registerTool`: `start_validation`, `get_validation_status`, `get_evidence`, `request_deep_scan`, `get_verdict`, `refine_idea`, `compare_ideas`, `get_next_step`, `list_repos`, `analyze_repo`, `rank_portfolio`, and `search_knowledge`. What that enables:
...
**Zero-budget engineering.** The entire product runs on $0/month:

- One free Render web container runs Next.js 15 *and* the pg-boss pipeline worker, colocated through the Next instrumentation hook — which also applies Prisma migrations and ingests the knowledge corpus at boot. No paid worker instance.
...
- Prisma pools are capped at 6 + 3 connections and pg-boss at 4 — a 13-connection worst case against free Postgres's ~95 — with 2-second in-process micro-caches collapsing poll bursts and a retention sweep keeping the database bounded.
...
**The two-judge integrity design.** The anti-hallucination story is enforced in code, not in prompts. Judge 1 may only cite evidence ids we actually supplied — ghost citations are stripped, and a score above 40 that loses its citations in that review is capped at 40 in code. Fewer than 2 evidence items caps a dimension at 45, regardless of what the model claims. Judge 2's composite adjustment is clamped to ±8, and the BUILD/PIVOT/KILL thresholds are re-applied in code after the adjustment. The model argues; the code rules.

**Security and privacy.** Anonymous visitors get a 256-bit capability cookie; only its SHA-256 hash is stored, with a 30-day sliding expiry. Per-visitor history isolation was adversarially audited, unowned resources return identical 404s, and a 100 trials/day/IP ceiling exists solely so a runaway script cannot burn the shared free inference quota. Evidence snippets are sanitized and framed as data, never instructions, before reaching the models.
...
- *Chrome 149+*: enable `chrome://flags/#enable-webmcp-testing`, restart, open the site, and use any WebMCP-capable agent surface. The 12 registered tools are inspectable in DevTools.
...
A judge access code, which exchanges for a cookie that lifts the daily trial ceiling, is provided separately in the submission form.
