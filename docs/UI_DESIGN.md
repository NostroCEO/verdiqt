...
1. Landing `app/page.tsx`: rail-bound editorial hero with an original halftone gavel, one-line promise, accessible public-repository or idea switch, "Open the case" CTA, selected technology wall, and one embedded interactive trial dashboard. The dashboard is application chrome, not a marketing explanation section or tutorial carousel. It has three 40 px phase controls, dashboard navigation, current-case state, system connections, evidence and score summaries, and locked result regions. Before the backend exists, only Phase 1 intake is active; evidence and verdict remain disabled. After Task 11, persisted worker status and SSE events alone advance the phases. The dashboard itself contains no autoplay or decorative animation. The completed decision state uses the verified Bklit gauge and radar components with real scores only. The technology wall uses official transparent marks for OpenAI, Next.js, Prisma, and Render only. Until the backend exists, a valid CTA submission dispatches the local preview-start event, loads the input into Phase 1, scrolls to the dashboard, and must not claim that a case or trial was created. Once the backend exists and the launch gates are closed, the CTA starts a real trial through the same `POST /api/trials` route used by the WebMCP tool, then navigates only after a successful response.
...
- The embedded trial dashboard never autoplays, loops, or uses Motion. Phase 1 is the only active phase before the backend exists. Phases 2 and 3 are disabled and visibly locked until real persisted trial statuses or SSE events unlock them. A valid hero submission loads Phase 1 and scrolls to the dashboard. Dashboard state changes are immediate. Do not render Replay, Simulated, or equivalent playback controls.
...
- The auto-cycling technology wall pauses for the documented hover and viewport conditions. The three trial status cells remain in one row at 375 px and introduce no horizontal overflow. Locked phases are not interactive or keyboard targets.
