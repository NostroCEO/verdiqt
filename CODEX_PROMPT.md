...
4. Verify the WebMCP registration API against https://developer.chrome.com/docs/ai/webmcp before writing the registry; the rules snippet shows document.modelContext and Chrome documents navigator.modelContext, so the registry feature-detects both. Tool results follow the dual format from https://developers.openai.com/api/docs/mcp.
